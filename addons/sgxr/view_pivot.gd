...
func _on_timer_timeout() -> void:
	...
	_pivot_tween.tween_method(self._tween_global_quaternion, gt.basis.get_rotation_quaternion(), new_basis.get_rotation_quaternion(), movement_delay) \
		.set_trans(Tween.TRANS_EXPO) \
		.set_ease(Tween.EASE_OUT)
	_pivot_tween.parallel().tween_property(self, "global_position", Vector3(_current_position.x, gt.origin.y, _current_position.z), movement_delay) \
		.set_trans(Tween.TRANS_EXPO) \
		.set_ease(Tween.EASE_OUT)
