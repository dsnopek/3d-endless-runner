@tool
class_name SGUIScreenContainer
extends Container
...
func show_screen(p_name: String, p_info: Dictionary = {}, p_reshow_screen := false) -> bool:
	...
	if not p_reshow_screen and screen == _current_screen_control:
		return true
...
func _get_minimum_size() -> Vector2:
	...
	for child in get_children():
		...
		#print("Child '%s' size of %s" % [child.name, cms])
